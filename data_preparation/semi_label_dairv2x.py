...
class DAIRV2XBaseDataset(Dataset):
    def __init__(self, root_dir, data_dir, semi_ratio=0.1):
        ...
        labeled_list = random.sample(self.split_info, int(len(self.split_info)* semi_ratio))
        unlabeled_list = [x for x in self.split_info if x not in labeled_list]

        label_path = root_dir.replace('train','train_labeled_{}'.format(semi_ratio))
        unlabel_path = root_dir.replace('train','train_unlabeled_{}'.format(semi_ratio))
        ...
        for veh_frame_id in labeled_list:
            ...
            labels = read_json(veh_label_path)
            self.ori_label_num += len(labels)
            self.labeled_num += len(labels)

            infra_label_path = os.path.join(self.root_dir, 'infrastructure-side/label/virtuallidar/{}.json'.format(inf_frame_id))
            labels = read_json(infra_label_path)
            self.ori_label_num += len(labels)
            self.labeled_num += len(labels)
        

        for veh_frame_id in unlabeled_list:
            frame_info = self.co_data[veh_frame_id]
            inf_frame_id = frame_info['infrastructure_image_path'].split("/")[-1].replace(".jpg", "")
            veh_label_path = os.path.join(self.root_dir, 'vehicle-side/label/lidar/{}.json'.format(veh_frame_id))
            labels = read_json(veh_label_path)
            self.ori_label_num += len(labels)
            self.unlabeled_num += len(labels)
        ...
        print(self.ori_label_num)
        print(self.labeled_num)
        print(self.unlabeled_num)
        print(semi_ratio, self.labeled_num / self.ori_label_num)

        with open(label_path, 'w') as f:
            json.dump(labeled_list, f)
        
        with open(unlabel_path, 'w') as f:
            json.dump(unlabeled_list, f)
...
if __name__ == '__main__':
    ...
    v2xsim_train = DAIRV2XBaseDataset(root_dir, data_dir, semi_ratio=0.05)    