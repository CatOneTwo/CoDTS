...
def load_saved_ema_model(saved_path, model, initial_epoch=-1):
    """
    Load saved model if exiseted
    ...
    if initial_epoch >= 0:
        ...
        model.load_state_dict(torch.load(
            os.path.join(saved_path,
                         'ema_net_epoch%d.pth' % initial_epoch), map_location='cpu'), strict=False)
        return initial_epoch, model


    def findLastCheckpoint(save_dir):
        file_list = glob.glob(os.path.join(save_dir, 'ema*epoch*.pth'))
        if file_list:
            epochs_exist = []
            for file_ in file_list:
                result = re.findall(".ema*epoch(.*).pth.*", file_)
                epochs_exist.append(int(result[0]))
            initial_epoch_ = max(epochs_exist)
        else:
            initial_epoch_ = 0
    ...
    file_list = glob.glob(os.path.join(saved_path, 'ema_net_epoch_bestval_at*.pth'))
    if file_list:
        assert len(file_list) == 1
        print("resuming best validation model at epoch %d" % \
                eval(file_list[0].split("/")[-1].rstrip(".pth").lstrip("ema_net_epoch_bestval_at")))
        model.load_state_dict(torch.load(file_list[0] , map_location='cpu'), strict=False)
        return eval(file_list[0].split("/")[-1].rstrip(".pth").lstrip("ema_net_epoch_bestval_at")), model
