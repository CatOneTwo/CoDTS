# -*- coding: utf-8 -*-
# Author: Runsheng Xu <rxx3386@ucla.edu>, Yifan Lu <yifan_lu@sjtu.edu.cn>, Yushan Han <21112034@bjtu.edu.cn>
# License: TDG-Attribution-NonCommercial-NoDistrib

# static teacher -> pseudo label, 同时生成ema_teacher CoDTS 版本
# 使用ema_teacher生成static teacher之外的标签
...
#   torch.use_deterministic_algorithms(True) # check所有torch操作的确定性
...
def train_parser():
    ...
    parser.add_argument('--nas', type=bool, default=True,
                        help='neighbor anchor sampling')
...
def main():
    ...
    static_teacher_model = train_utils.load_pretrained_model(opt.teacher_model, static_teacher_model) 
    ...
    for epoch in range(init_epoch, max(epoches, init_epoch)):
        ...
        for i, batch_data in enumerate(train_loader):
            ...
            model.train()
            model.zero_grad() # 将所有模型参数的梯度置为0
            optimizer.zero_grad() # 清除所有优化的torch.Tensor的梯度
            batch_data = train_utils.to_device(batch_data, device)
            batch_data['ego']['epoch'] = epoch
            ouput_dict = model(batch_data['ego'])
            
            # hys add
            ################# static teacher 赋予每个scene伪gt ##################
            ego_data = batch_data['ego']

            teacher_ouput_dict = static_teacher_model(ego_data)
            ...
            ################# generate dual threshold for Teacher ##################
            # # 稀疏标签生成的label
            # pos_equal_one = ego_data['label_dict']['pos_equal_one'].permute(0, 3, 1, 2) # [b, h, w, 2]->[b, 2, h, w]
            # # 协同端端预测
            # foreground_maps_ego = batch_cls_preds.sigmoid() # [b, 2, h,w]
            # # 统计预测gt的分数
            # gt_pred_score = foreground_maps_ego[pos_equal_one > 0] # 
            # if len(gt_pred_score) == 1:
            #     score_thre_high, score_thre_low  = float(gt_pred_score[0]), float(gt_pred_score[0])
            # else:
            #     score_thre_high, score_thre_low = get_jnb_threshold(gt_pred_score.tolist()) # high thre, low thre
            # st_score_thre = score_thre_high if auxiliary_teacher else score_thre_low
            # print('st', st_score_thre)

            
            # 辅助教师的预测
            if auxiliary_teacher:
                if opt.ema_usage_way=='epoch':
                    teacher_ouput_dict_2 = ema_teacher_model_copy(ego_data) # 使用上一轮固定的ema teacher
                ...
                if len(gt_pred_score) == 1:
                    ...
                    score_thre_2, score_thre_2_low = get_jnb_threshold(gt_pred_score.tolist()) # high thre, low thre
                print('dt',score_thre_2, score_thre_2_low)
            ...
            for item in range(bs):
                ...
                if pred_center is None:
                    ...
                    pseudo_object_bbx_center = np.zeros((1000, 7))
                    pseudo_mask = np.zeros(1000)               
                    pseudo_object_bbx_center[:pred_center.size(0), :] = pred_center.cpu()
                    pseudo_mask[:pred_center.size(0)] = 1
                    
                    # 是否用neighbor anchor sampling
                    if not opt.nas:
                        # neighbor_pos_threshold=1 neighbor_neg_threshold=0
