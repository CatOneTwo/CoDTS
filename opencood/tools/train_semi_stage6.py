    ...
    for epoch in range(init_epoch, max(epoches, init_epoch)):
        ...
        for i, batch_data in enumerate(train_loader_labeled):
            ...
            if epoch > 4:
                ...
                ego_data_copy = copy.deepcopy(batch_data['ego'])
                ego_data_copy = train_utils.to_device(ego_data_copy, device)
                pos_equal_one = ego_data_copy['label_dict']['pos_equal_one'].permute(0, 3, 1, 2) # [b, h, w, 2]->[b, 2, h, w]                
                teacher_ouput_dict = ema_teacher_model(ego_data_copy)
                # 协同端端预测
                ...
                downsampled_voxel_features = batch_data['ego']['downsampled_processed_lidar']['voxel_features'] # (num_voxels, 32, 4)
                downsampled_voxel_coords = batch_data['ego']['downsampled_processed_lidar']['voxel_coords'] # (num_voxels, 4), [batch_idx, z_idx, y_idx, x_idx]
                downsampled_voxel_num_points = batch_data['ego']['downsampled_processed_lidar']['voxel_num_points']
                ...
                downsampled_voxel_features_unlabeled = batch_data_unlabeled['ego']['downsampled_processed_lidar']['voxel_features'] # (num_voxels, 32, 4)
                downsampled_voxel_coords_unlabeled = batch_data_unlabeled['ego']['downsampled_processed_lidar']['voxel_coords'] # (num_voxels, 4), [batch_idx, z_idx, y_idx, x_idx]
                downsampled_voxel_num_points_unlabeled = batch_data_unlabeled['ego']['downsampled_processed_lidar']['voxel_num_points']
                ...
                downsampled_voxel_features_new = torch.cat((downsampled_voxel_features,downsampled_voxel_features_unlabeled),dim=0).to(device)
                downsampled_voxel_coords_unlabeled_= copy.deepcopy(downsampled_voxel_coords_unlabeled)# 第一维维batch id
                downsampled_voxel_coords_unlabeled_[:,0] += torch.sum(record_len)# 第一维维batch id
                downsampled_voxel_coords_new = torch.cat((downsampled_voxel_coords,downsampled_voxel_coords_unlabeled_),dim=0).to(device)
                downsampled_voxel_num_points_new = torch.cat((downsampled_voxel_num_points,downsampled_voxel_num_points_unlabeled),dim=0).to(device)
                ...
                batch_data['ego']['downsampled_processed_lidar']['voxel_features'] = downsampled_voxel_features_new
                batch_data['ego']['downsampled_processed_lidar']['voxel_coords'] = downsampled_voxel_coords_new
                batch_data['ego']['downsampled_processed_lidar']['voxel_num_points'] = downsampled_voxel_num_points_new
                ########## 2. 将标记和未标记数据拼接 ##########
                ...
                # generate corners_3d_pred for teacher model
                ...
                for item in range(bs):
                    ...
                    if pred_center is None:
                        ...
                        pseudo_object_bbx_center = np.zeros((400, 7))
                        pseudo_mask = np.zeros(400)                
                        pseudo_object_bbx_center[:pred_center.size(0), :] = pred_center.cpu()
                        pseudo_mask[:pred_center.size(0)] = 1
                        item_pseudo_label_dict = opencood_train_dataset_labeled.post_processor.generate_label(
                            gt_box_center=pseudo_object_bbx_center, 
                ...
                batch_data['ego']['label_dict']['mid_level_pos_equal_one'] = torch.cat((pos_equal_one, mid_level_pseudo_pos_equal_one),dim=0).to(device) 
                pos_equal_one_weight = torch.ones_like(mid_level_pseudo_pos_equal_one_weight).to(device)
                batch_data['ego']['label_dict']['mid_level_pos_equal_one_weight'] = torch.cat((pos_equal_one_weight, mid_level_pseudo_pos_equal_one_weight),dim=0).to(device)  
                batch_data['ego']['label_dict']['mid_level_neg_equal_one'] = torch.cat((neg_equal_one, mid_level_pseudo_neg_equal_one),dim=0).to(device) 
                batch_data['ego']['label_dict']['mid_level_targets'] = torch.cat((targets,mid_level_pseudo_target),dim=0) 
        ...
        if epoch % hypes['train_params']['eval_freq'] == 0:
            ...
            with torch.no_grad():
                for i, batch_data in enumerate(val_loader):
                    ...
                    final_loss = criterion(ouput_dict, batch_data['ego']['label_dict'],training=False)
