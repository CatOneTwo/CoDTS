...
class BasePostprocessor(object):
    ...
    def generate_gt_bbx(self, data_dict):
        ...
        for cav_id, cav_content in data_dict.items():
            ...
            transformation_matrix = cav_content['transformation_matrix_clean'] # 中期协同为1

            object_bbx_center = cav_content['object_bbx_center'] # [1,100,7]
            object_bbx_mask = cav_content['object_bbx_mask'] # [1,100] 有目标的地方为1，其余为0
            object_ids = cav_content['object_ids'] # list 车辆编号
            object_bbx_center = object_bbx_center[object_bbx_mask == 1] # [n,7] n 为目标数量

            # convert center to corner
            object_bbx_corner = \
                box_utils.boxes_to_corners_3d(object_bbx_center,
                                              self.params['order']) # 转换为角点 [n,8,3]
            projected_object_bbx_corner = \
                box_utils.project_box3d(object_bbx_corner.float(),
                                        transformation_matrix) # 转换到ego坐标下 [n,8,3] 中期协同转换后位置不变
            gt_box3d_list.append(projected_object_bbx_corner)
            # append the corresponding ids
            object_id_list += object_ids

        # gt bbx 3d
        gt_box3d_list = torch.vstack(gt_box3d_list) # [n,8,3]
        # some of the bbx may be repetitive, use the id list to filter
        gt_box3d_selected_indices = \
            [object_id_list.index(x) for x in set(object_id_list)] # [0,1,2,...,n-1]
        ...
        gt_box3d_np = gt_box3d_tensor.cpu().numpy() # (n,8,3)
