...
def getIntermediateFusionDataset(cls):
    ...
    class IntermediateFusionDataset(cls):
        ...
        def post_process_for_pseudo_label(self, data_dict, output_dict):
            """
            ...
            gt_box_tensor : torch.Tensor
                The tensor of gt bounding box.
            """
            output_dict['ego'].update({'mask_index': True})
            pred_box_tensor, pred_score, mask_index = \
                self.post_processor.post_process(data_dict, output_dict)
            gt_box_tensor = self.post_processor.generate_gt_bbx(data_dict)

            return pred_box_tensor, pred_score, gt_box_tensor, mask_index
