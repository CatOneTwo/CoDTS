# intermediate fusion dataset for evaluation in sparse supervised learning
import random
import math
...
def getSsintermediateFusionDataset(cls):
    """
    cls: the Basedataset.
    """
    class SSIntermediateFusionDataset(cls):
        ...
        def get_item_single_car(self, selected_cav_base, ego_cav_base):
            ...
            # hys add : Use the fully supervised label to evaluate the quality of the pseudo label
            if self.label_sparse and self.pseudo_label_eval:
                selected_cav_base['params']['vehicles'] = selected_cav_base['params']['vehicles_fs']
                if 'v2xsim' in self.root_dir:
                    selected_cav_base['params']['object_ids'] = selected_cav_base['params']['object_ids_fs']

                object_bbx_center_fs, object_bbx_mask_fs, object_ids_fs = self.generate_object_center([selected_cav_base],ego_pose_clean)
                selected_cav_processed.update(
                    {
                        "object_bbx_center_fs": object_bbx_center_fs[object_bbx_mask_fs == 1],
                        "object_bbx_mask_fs": object_bbx_mask_fs,
                        "object_ids_fs": object_ids_fs
                    }
                )

                if self.supervise_single:
                    # generate targets label single GT, note the reference pose is itself.
                    object_bbx_center_fs_single, object_bbx_mask_fs_single, object_ids_fs_single = self.generate_object_center_single(
                        [selected_cav_base], selected_cav_base['params']['lidar_pose']
                    )
                    label_dict_fs_single = self.post_processor.generate_label(
                        gt_box_center=object_bbx_center_fs_single, anchors=self.anchor_box, mask=object_bbx_mask_fs_single
                    )
                    selected_cav_processed.update({
                                        "single_label_dict_fs": label_dict_fs_single,
                                        "single_object_bbx_center_fs": object_bbx_center_fs_single,
                                        "single_object_bbx_mask_fs": object_bbx_mask_fs_single})

            return selected_cav_processed

        def __getitem__(self, idx):
            base_data_dict = self.retrieve_base_data(idx) # each agent ['ego', 'params', 'lidar_np']
            ...
            object_stack_fs = []
            object_id_stack_fs = []
            single_label_list_fs = []
            single_object_bbx_center_list_fs = []
            single_object_bbx_mask_list_fs = []
            ...
            for _i, cav_id in enumerate(cav_id_list):
                ...
                # Use the fully supervised label to evaluate the quality of the pseudo label
                if self.label_sparse and self.pseudo_label_eval:
                    object_stack_fs.append(selected_cav_processed['object_bbx_center_fs'])
                    object_id_stack_fs += selected_cav_processed['object_ids_fs']
                ...
                if self.supervise_single:
                    ...
                    # Use the fully supervised label to evaluate the quality of the pseudo label
                    if self.label_sparse and self.pseudo_label_eval:
                        single_label_list_fs.append(selected_cav_processed['single_label_dict_fs'])
                        single_object_bbx_center_list_fs.append(selected_cav_processed['single_object_bbx_center_fs'])
                        single_object_bbx_mask_list_fs.append(selected_cav_processed['single_object_bbx_mask_fs'])
            ...
            if self.supervise_single:
                ...
                if self.label_sparse and self.pseudo_label_eval:
                    single_label_dicts_fs = self.post_processor.collate_batch(single_label_list_fs)
                    single_object_bbx_center_fs = torch.from_numpy(np.array(single_object_bbx_center_list_fs))
                    single_object_bbx_mask_fs = torch.from_numpy(np.array(single_object_bbx_mask_list_fs))
                    processed_data_dict['ego'].update({
                        "single_label_dict_torch_fs": single_label_dicts_fs,
                        "single_object_bbx_center_torch_fs": single_object_bbx_center_fs,
                        "single_object_bbx_mask_torch_fs": single_object_bbx_mask_fs,
                        })
            ...
            if self.label_sparse and self.pseudo_label_eval:
                # exclude all repetitive objects    
                unique_indices_fs = \
                    [object_id_stack_fs.index(x) for x in set(object_id_stack_fs)]
                object_stack_fs = np.vstack(object_stack_fs)
                object_stack_fs = object_stack_fs[unique_indices_fs]

                # make sure bounding boxes across all frames have the same number
                object_bbx_center_fs = \
                    np.zeros((self.params['postprocess']['max_num'], 7))
                mask_fs = np.zeros(self.params['postprocess']['max_num'])
                object_bbx_center_fs[:object_stack_fs.shape[0], :] = object_stack_fs
                mask_fs[:object_stack_fs.shape[0]] = 1

                # generate targets label
                label_dict_fs = \
                    self.post_processor.generate_label(
                        gt_box_center=object_bbx_center_fs,
                        anchors=self.anchor_box,
                        mask=mask_fs)

                processed_data_dict['ego'].update(
                    {'object_bbx_center_fs': object_bbx_center_fs,
                    'object_bbx_mask_fs': mask_fs,
                    'object_ids_fs': [object_id_stack_fs[i] for i in unique_indices_fs],
                    'label_dict_fs': label_dict_fs})
        ...
        def collate_batch_train(self, batch):
            ...
            for i in range(len(batch)):
                ...
                # Use the fully supervised label to evaluate the quality of the pseudo label
                if self.label_sparse and self.pseudo_label_eval:
                    object_bbx_center_fs.append(ego_dict['object_bbx_center_fs'])
                    object_bbx_mask_fs.append(ego_dict['object_bbx_mask_fs'])
                    object_ids_fs.append(ego_dict['object_ids_fs'])
                    label_dict_list_fs.append(ego_dict['label_dict_fs'])
            ...
            if self.label_sparse and self.pseudo_label_eval:
                # convert to numpy, (B, max_num, 7)
                object_bbx_center_fs = torch.from_numpy(np.array(object_bbx_center_fs))
                object_bbx_mask_fs = torch.from_numpy(np.array(object_bbx_mask_fs))
                label_torch_dict_fs = \
                    self.post_processor.collate_batch(label_dict_list_fs)
                # for centerpoint
                label_torch_dict_fs.update({'object_bbx_center_fs': object_bbx_center_fs,
                                        'object_bbx_mask_fs': object_bbx_mask_fs})
                # add pairwise_t_matrix to label dict
                label_torch_dict_fs['pairwise_t_matrix'] = pairwise_t_matrix
                label_torch_dict_fs['record_len'] = record_len
                
                output_dict['ego'].update({'object_bbx_center_fs': object_bbx_center_fs,
                                    'object_bbx_mask_fs': object_bbx_mask_fs,
                                    'label_dict_fs': label_torch_dict_fs,
                                    'object_ids_fs': object_ids_fs})
    ...
    return SSIntermediateFusionDataset
