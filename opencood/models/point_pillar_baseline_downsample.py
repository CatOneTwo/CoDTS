...
class PointPillarBaselineDownsample(nn.Module):
    """
    F-Cooper implementation with point pillar backbone.
    """
    def __init__(self, args):
        super(PointPillarBaselineDownsample, self).__init__()
    ...
    def se_block(self, feature):
        #[2, 256, 200, 176])
        b, c, _, _ = feature.size()
        y = self.avg_pool(feature).view(b, c)
        y = self.fc(y)
        #y = self.fc(y).view(b, 1, 1, 1)
        # return feature * y.expand_as(feature)
        return y
    ...
    def forward(self, data_dict):
        ...
        if self.training:
            # 下采样点云
            downsampled_voxel_features = data_dict['downsampled_processed_lidar']['voxel_features'] # (num_voxels, 32, 4)
            downsampled_voxel_coords = data_dict['downsampled_processed_lidar']['voxel_coords'] # (num_voxels, 4), [batch_idx, z_idx, y_idx, x_idx]
            downsampled_voxel_num_points = data_dict['downsampled_processed_lidar']['voxel_num_points']

            downsampled_batch_dict = {'voxel_features': downsampled_voxel_features,
                        'voxel_coords': downsampled_voxel_coords, 
                        'voxel_num_points': downsampled_voxel_num_points,
                        'record_len': record_len}
            
            # n, 4 -> n, cc
            downsampled_batch_dict = self.pillar_vfe(downsampled_batch_dict)
            # n, c -> N, C, H, W
            downsampled_batch_dict = self.scatter(downsampled_batch_dict) # sptial_features [b, 64, 200, 504] voxel 尺寸
            downsampled_batch_dict = self.backbone(downsampled_batch_dict)        
            
            downsampled_spatial_features_2d = downsampled_batch_dict['spatial_features_2d'] # [b, 384, 100, 252]

            if self.shrink_flag:
                downsampled_spatial_features_2d = self.shrink_conv(downsampled_spatial_features_2d) # [b, 256, 100, 252] channel维度压缩
            if self.compression:
                downsampled_spatial_features_2d = self.naive_compressor(downsampled_spatial_features_2d)

            downsampled_fused_feature = self.fusion_net(downsampled_spatial_features_2d, record_len, t_matrix) # [b_ego, 256, 100, 252]

            downsampled_psm = self.cls_head(downsampled_fused_feature) # [b_ego, 2, 100, 252]
            downsampled_rm = self.reg_head(downsampled_fused_feature) # [b_ego, 14, 100, 252]

            output_dict.update({'downsampled_cls_preds': downsampled_psm,
                                'downsampled_reg_preds': downsampled_rm})
            if self.use_dir:
                output_dict.update({'downsampled_dir_preds': self.dir_head(downsampled_fused_feature)}) # [b_ego, 4, 100, 252]

            # mixed-density feature fusion module
            fused_feature_org = fused_feature.clone()
            lambda_1 = self.se_block(downsampled_fused_feature)
            lambda_2 = self.se_block(fused_feature_org)
            lambda_1_ = ((lambda_1 / (lambda_1 + lambda_2)).sum())/2
            lambda_2_ = 1. - lambda_1_
            mixed_fused_feature = downsampled_fused_feature * lambda_1_ + fused_feature_org * lambda_2_
            # mixed_fused_feature = (fused_feature + downsampled_fused_feature)/2

            mixed_psm = self.cls_head(mixed_fused_feature) # [b_ego, 2, 100, 252]
            mixed_rm = self.reg_head(mixed_fused_feature) # [b_ego, 14, 100, 252]

            output_dict.update({'mixed_cls_preds': mixed_psm,
                                'mixed_reg_preds': mixed_rm})
            if self.use_dir:
                output_dict.update({'mixed_dir_preds': self.dir_head(mixed_fused_feature)}) # [b_ego, 4, 100, 252]
