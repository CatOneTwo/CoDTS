...
def warp_affine_simple(src, M, dsize,
    ...
    # M [bs,2,3]
    # src [bs, C, H, W] 
    B, C, H, W = src.size() # feature size

    # F.affine_grid(theta, size)  theta(n,2,3) size(n,c,h,w)->output: (n,h,w,2)
    grid = F.affine_grid(M,
                         [B, C, dsize[0], dsize[1]],
                         align_corners=align_corners).to(src) # 根据几何变换（2Dor3D仿射变换）生成一个grid用于grid sample [bs, H, W, 2]
    # F.grid_sample(input,grid) input(N,C,Hin,Win) grid(N,Hout,Wout,2) -> output(N,C,Hout,Wout)
