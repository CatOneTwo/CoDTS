...
class PillarVFE(nn.Module):
    ...
    def get_paddings_indicator(actual_num, max_num, axis=0):
        ...
        max_num = torch.arange(max_num,
                               dtype=torch.int,
                               device=actual_num.device).view(max_num_shape) # [0,1,2,...,31]
        paddings_indicator = actual_num.int() > max_num # [M,32] 每个voxel有点的为True，没有点的为False
        return paddings_indicator

    def forward(self, batch_dict):
        """encoding voxel feature using point-pillar method
        Args:
            voxel_features: [M, 32, 4] ->xyzr
            voxel_num_points: [M,]
            voxel_coords: [M, 4]
        Returns:
        ...
        points_mean = \
            ...
            voxel_num_points.type_as(voxel_features).view(-1, 1, 1) # voxel点云中心的位置[M, 1, 3]
        f_cluster = voxel_features[:, :, :3] - points_mean # voxel各点到中心位置的距离 [M,32,3]
        ...
        features *= mask # 每个voxel未必填满，将没有填满的点置0
